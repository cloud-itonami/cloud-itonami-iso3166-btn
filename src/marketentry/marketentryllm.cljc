(ns marketentry.marketentryllm
  ...
  the Kingdom of Bhutan (BTN) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real e-GP tender response / FDI
  Registration Certificate application. Every output is censored
  downstream by `marketentry.governor` before anything touches the
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world e-GP tender response / FDI
  Registration Certificate application submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "tpn-registered?=" (:tpn-registered? e)
                        " foreign-company?=" (:foreign-company? e)
                        " fdi-sector=" (:fdi-sector e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tpn-registration? e))
                              (:tpn-registered? e)))
                   0.9 0.3)}))
