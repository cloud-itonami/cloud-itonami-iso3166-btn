(ns marketentry.store
  "SSoT for the Kingdom of Bhutan (BTN) market-entry compliance actor,
...
(defn demo-data
  ...
  `:foreign-company?` / `:fdi-sector` are ground truth for the flagship
  SET-MEMBERSHIP check (FDI Rules and Regulations 2025 Rule 4 + Schedule
  III Negative List -- is the declared sector barred for an FDI
  Company?); `:requires-tpn-registration?` / `:tpn-registered?` are
  ground truth for the conditional Department of Revenue and Customs
  (DRC) Taxpayer Number (TPN) check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Thimphu Trading House Pvt Ltd" :procurement-channel "e-GP (egp.gov.bt) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-company? true
             :fdi-sector "IT/IT Enabled Services (Outside IT Park)"
             :requires-tpn-registration? true :tpn-registered? true
             :drafted? false :submitted? false
             :jurisdiction "BTN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :procurement-channel "e-GP (egp.gov.bt) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-company? true
             :fdi-sector "IT/IT Enabled Services (Outside IT Park)"
    ...
    "eng-3" {:id "eng-3" :operator "Paro Ventures Pvt Ltd" :procurement-channel "e-GP (egp.gov.bt) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BTN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Gelephu Real Estate Holdings Ltd" :procurement-channel "e-GP (egp.gov.bt) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-company? true
             :fdi-sector "Real Estate Businesses"
             :requires-tpn-registration? true :tpn-registered? true
             :drafted? false :submitted? false
             :jurisdiction "BTN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Wangdue Construction Pvt Ltd" :procurement-channel "e-GP (egp.gov.bt) tender notice"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :foreign-company? true
             :fdi-sector "IT/IT Enabled Services (Outside IT Park)"
             :requires-tpn-registration? true :tpn-registered? false
             :drafted? false :submitted? false
             :jurisdiction "BTN" :status :intake}
    "eng-6" {:id "eng-6" :operator "Punakha Textiles Pvt Ltd" :procurement-channel "e-GP (egp.gov.bt) tender notice"
...
(defn- engagement->tx [{:keys [id operator procurement-channel base-fee monthly-rate monitoring-months claimed-fee
                               foreign-company? fdi-sector
                               requires-tpn-registration? tpn-registered?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    fdi-sector                                      (assoc :engagement/fdi-sector fdi-sector)
    (some? requires-tpn-registration?)             (assoc :engagement/requires-tpn-registration? requires-tpn-registration?)
    (some? tpn-registered?)                        (assoc :engagement/tpn-registered? tpn-registered?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/procurement-channel :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/foreign-company? :engagement/fdi-sector
   :engagement/requires-tpn-registration? :engagement/tpn-registered?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :procurement-channel (:engagement/procurement-channel m)
     ...
     :fdi-sector (:engagement/fdi-sector m)
     :requires-tpn-registration? (boolean (:engagement/requires-tpn-registration? m))
     :tpn-registered? (boolean (:engagement/tpn-registered? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-btn"})
