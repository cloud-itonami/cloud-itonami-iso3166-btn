...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (BTN, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up FDI sector restricted) ==")
    ...
    (println "== filing/submit eng-4 (FDI sector on Schedule III Negative List -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up TPN-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (TPN registration unverified -> HARD hold) ==")
