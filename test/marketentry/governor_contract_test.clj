  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Thimphu Trading House Pvt Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Thimphu Trading House Pvt Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest fdi-sector-restricted-is-held-and-unoverridable
  (testing "FDI Rules and Regulations 2025 Schedule III Negative List membership ('Real Estate Businesses') -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:fdi-sector-restricted} (-> (store/ledger db) last :basis)))
...
(deftest tpn-registration-unverified-is-held-and-unoverridable
  (testing "unverified Department of Revenue and Customs TPN registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tpn-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest domestic-company-is-not-gated-by-fdi-negative-list-check
  (testing "eng-6 is a domestic Bhutanese company (:foreign-company? false) declaring a Negative-List sector ('Real Estate Businesses') -- the flagship check never fires for it, because the FDI Rules and Regulations 2025 (Rule 138(12)) only govern an FDI Company"
    (let [[_db actor] (fresh)
